...
if($count[0][0] == 0) {
    for ($i = 1; $i < 12; $i++) {
        ...
        for($j = 0; $j < $last; $j++) {
            ...
            $number = rand(20, 35);
            $school_childs = new School_class($i, $last_name, $number);
            mysqli_query($connect, "INSERT INTO school (first_name, last_name, number) VALUES ('{$school_childs->first_name}', '{$school_childs->last_name}', {$school_childs->number})");
        }
    }
}
...
if($count2[0][0] == 0) {
    for ($k = 0; $k < count($school); $k++) {
        ...
        for ($l = 0; $l < $class_info["number"]; $l++) {
            ...
            $city = "\"$faker->city\"";
            $schoolchild = new Schoolchild($name, $surname, $city, $class_id[0]);
            mysqli_query($connect, "INSERT INTO schoolchilds (name, surname, city, class_id) VALUES ({$schoolchild->name}, {$schoolchild->surname}, {$schoolchild->city}, {$schoolchild->class_id})");
        }
    }
}
